...
namespace ash {
        ...
        void processLoop() {
            while (running) {
                ...
                while (!messageQueue.empty()) {
                    LogMessage msg = std::move(messageQueue.front());
                    messageQueue.pop();
                    lock.unlock();
                    ...
                    switch (msg.level) {
                        ...
                        default: colorStart = "\033[37m"; break;
                    }

                    {
                        std::scoped_lock outLock(outputMutex);
                        if (isConsole(out))
                            *out << colorStart << msg.text << colorEnd << '\n';
                        else
                            *out << msg.text << '\n';
                        out->flush();
                    }
    ...
    class LogStream {
    public:
        explicit LogStream(LogLevel level) : m_level(level) {}
        ~LogStream() {
