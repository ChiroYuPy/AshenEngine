#include "Ashen/Graphics/CameraControllers/Camera2DController.h"

#include "Ashen/Events/EventDispatcher.h"

namespace ash {
    Ref<Camera2DController> Camera2DController::Create(OrthographicCamera &camera, const float moveSpeed,
    ...
    void Camera2DController::HandleMouseScrolledEvent(const MouseScrolledEvent &mouseScrolled) const {
    ...
    void Camera2DController::Update(const float deltaTime) {
        ...
        const auto *orthoCamera = dynamic_cast<OrthographicCamera *>(&m_Camera);
        if (!orthoCamera) return;
    ...
    void Camera2DController::ApplyBounds() const {
        Vec3 position = m_Camera.GetPosition();
