...
// NE PAS définir MINIAUDIO_IMPLEMENTATION ici !
#include <miniaudio.h>

namespace ash {

    MiniAudioSource::MiniAudioSource(ma_engine* engine, const std::string& filepath,
                                     const AudioSourceConfig& config, const AudioSourceHandle handle)
        : m_Engine(engine)
        ...
        // Charger le son
        ma_result result = ma_sound_init_from_file(m_Engine, filepath.c_str(), flags, nullptr, nullptr, m_Sound);

        if (result != MA_SUCCESS) {
            Logger::Error("Failed to load audio file: {} (error code: {})", filepath, static_cast<int>(result));
            delete m_Sound;
            m_Sound = nullptr;
            return;
        }

        Logger::Debug("Audio file loaded: {}", filepath);
    ...
    void MiniAudioSource::Play() {
        if (m_Sound) {
            // Remettre au début si le son est arrêté
    ...
    void MiniAudioSource::Pause() {
        if (m_Sound && IsPlaying()) {
            // Note: miniaudio n'a pas de pause native, on utilise stop
            // mais on garde la position actuelle
    ...
    AudioState MiniAudioSource::GetState() const {
        ...
        // Si le son est à la fin, il est arrêté
        if (ma_sound_at_end(m_Sound)) {
