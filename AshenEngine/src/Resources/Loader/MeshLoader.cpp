...
namespace ash {
    Mesh MeshLoader::Load(const fs::path &path, bool flipUVs) {
        ...
        const String mtlBasedir = path.parent_path().string();
        ...
        std::unordered_set<String> materialNames;
        ...
        HashMap<VertexKey, uint32_t> vertexMap;
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f) {
            ...
            } else if (static_cast<size_t>(matId) >= materials.size()) {
            ...
            for (size_t v = 0; v < 3; ++v) {
                ...
                VertexKey key{vertex_index, normal_index, texcoord_index};
                ...
                if (it != vertexMap.end()) {
                    ...
                    Optional<Vec2> texCoord;
                    if (texcoord_index >= 0 && !attrib.texcoords.empty()) {
                        float u = attrib.texcoords[2 * texcoord_index + 0];
                        float v = attrib.texcoords[2 * texcoord_index + 1];
                        if (flipUVs) {
                            v = 1.0f - v;
                        }
                        texCoord = Vec2{u, v};
